...
def process_features(train,test):
    ...
    total_missing=train.isnull().sum() #counts the number of missing values for each attribute
    ...
    features = get_features(train,test) # returns list containing id of attributes common in both test and tr
...
def train_and_test_Kfold(train,test,features,target='SalePrice'): # add Kfold
    ...
    for eta,max_depth in list(itertools.product(eta_list, max_depth_list)): # Loop over parameters to find the better set
        ...
        fold=0
        fold_score=[]
        for train_index, cv_index in kf: # training and cross val data
